...
def calculate_recommendation_score(inputs):
    ...
    scores = {"US Providers": 0, "EU Providers": 0, "China Providers": 0}
...
def display_decision_helper():
    ...
    with col1:
        ...
        region = st.selectbox(
            "Primary Region of Operation", ["North America", "Europe", "Asia", "Global"]
        )

    with col2:
        budget_constraint = st.select_slider(
            "Budget Constraint Level", options=["Low", "Medium", "High"]
        )
    ...
    if any(inputs["tech_requirements"].values()):
        considerations.append(
            "- Verify specific technical capabilities with provider documentation"
        )
